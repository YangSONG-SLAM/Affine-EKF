function X_estimation=pointfeature_RIEKF(X0,P0,z0,U_noise,z_noise,Index,ODOM_noise,OBSV_noise)
% coded by Yang SONG
tic
T_steps=size(Index,2);
[Xn,Pn]=Augmentation_point(X0,P0,z0,OBSV_noise);
...
%     A(4:6,1:3)=skew( Xn(1:3,3*N+4) )*Xn(1:3,1:3);%this is for Lie group noise
    ...
    C=zeros(3*N,6);
    for j=1:N
        C(3*j-2:3*j,1:3)=skew( Xn(1:3,4+j) )*Xn(1:3,1:3);        
    end
...
%         Y(6*j-2:6*j,1)=z_noise{i}.position(1:3,j)-X_Prediction(1:3,1:3)'*(X_Prediction(1:3,3*N+4+j)-X_Prediction(1:3,3*N+4));
    ...
    Xn=poseState_plus_point(posestate_exp_point(K*Y),X_Prediction);
...
%         z_new=[z_noise{i}.rotation(1:3,3*N+1:end) z_noise{i}.position(1:3,N+1:end)];
        [Xn,Pn]=Augmentation_point(Xn,Pn,z_new,OBSV_noise);
        Pn=(Pn+Pn')/2;
    end
