function X_estimation=conpointfeature2_AffEKF(X0,P0,z0,U_noise,z_noise,Index,ODOM_noise,OBSV_noise)
...
%         H(3*j-2:3*j,3*Index{i}.RemainIndex(j)+1:3*Index{i}.RemainIndex(j)+3)=X_Prediction(1:3,1:3)';
        ...
        H(3*j-2:3*j,7)=X_Prediction(1:3,1:3)'*[0 0 1]';
        H(3*j-2:3*j,6+2*Index{i}.RemainIndex(j):7+2*Index{i}.RemainIndex(j))=X_Prediction(1:3,1:3)'*[eye(2);zeros(1,2)];
...
%         Y(6*j-2:6*j,1)=z_noise{i}.position(1:3,j)-X_Prediction(1:3,1:3)'*(X_Prediction(1:3,3*N+4+j)-X_Prediction(1:3,3*N+4));
    ...
    for j=1:N
        AA(6+2*j:7+2*j,3)=-J*( X_Prediction(1:2,4+j)-Xn(1:2,4+j) );
    end
    Pn=AA*Pn*AA';
