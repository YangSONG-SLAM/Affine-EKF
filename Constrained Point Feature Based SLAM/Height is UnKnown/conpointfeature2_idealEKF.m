function X_estimation=conpointfeature2_idealEKF(X0,P0,z0,U_noise,z_noise,Index,ODOM_noise,OBSV_noise,Xstate_gt)
%StdEKF:SO(3)-EKF, the state space is (SO(d)*R^d)^{N+1}
% coded by Yang SONG
tic
T_steps=size(Index,2);
[Xn,Pn]=Augmentation_Std_conpoint2(X0,P0,z0,OBSV_noise);
% Pn=0*Pn;
...
for i =1:T_steps
    ...
    %Prediction
    %The form of error xi is a column (xi_R,xi_x,z,xi_p)
    X_Prediction=Xn;
...
%     P_adX_W=[A*ODOM_noise*A' A*ODOM_noise*C';C*ODOM_noise'*A' C*ODOM_noise*C'];
    P_adX_W=blkdiag(A*ODOM_noise*A',zeros(2*N+1,2*N+1));
...
%         H(3*j-2:3*j,3*Index{i}.RemainIndex(j)+1:3*Index{i}.RemainIndex(j)+3)=X_Prediction(1:3,1:3)';
        ...
        H(3*j-2:3*j,7)=Xstate_gt{i}(1:3,1:3)'*[0 0 1]';
        H(3*j-2:3*j,6+2*Index{i}.RemainIndex(j):7+2*Index{i}.RemainIndex(j))=Xstate_gt{i}(1:3,1:3)'*[eye(2);zeros(1,2)];
...
%         Y(6*j-2:6*j,1)=z_noise{i}.position(1:3,j)-X_Prediction(1:3,1:3)'*(X_Prediction(1:3,3*N+4+j)-X_Prediction(1:3,3*N+4));
    ...
    Xn=poseState_Std_plus_point(posestate_Std_exp_conpoint2(K*Y),X_Prediction);
    Pn=(eye(2*N+7)-K*H)*Pn_Prediction;
...
%         [Xn,Pn]=Augmentation_Std_conpoint2(Xn,Pn,z_new,OBSV_noise);
        [Xn,Pn]=Augmentation_ideal_conpoint2(Xn,Pn,z_new,OBSV_noise,Xstate_gt{i});
        Pn=(Pn+Pn')/2;
    end
