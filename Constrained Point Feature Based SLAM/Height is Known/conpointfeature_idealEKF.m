function X_estimation=conpointfeature_idealEKF(X0,P0,z0,U_noise,z_noise,Index,ODOM_noise,OBSV_noise,fz_c,Xstate_gt)
...
%         H(3*j-2:3*j,3*Index{i}.RemainIndex(j)+1:3*Index{i}.RemainIndex(j)+3)=X_Prediction(1:3,1:3)';
        ...
        H(3*j-2:3*j,5+2*Index{i}.RemainIndex(j):6+2*Index{i}.RemainIndex(j))=Xstate_gt{i}(1:3,1:3)'*[eye(2);zeros(1,2)];
...
%         [Xn,Pn]=Augmentation_Std_conpoint(Xn,Pn,z_new,OBSV_noise,fz_c);
        [Xn,Pn]=Augmentation_ideal_conpoint(Xn,Pn,z_new,OBSV_noise,fz_c,Xstate_gt{i});
        Pn=(Pn+Pn')/2;
    end
