...
function NEES_add=conpointfeature_NEES_Std_add(X_Estimation,Xstate_gt,m)
...
for i=1:T_steps
    ...
    for j=1:N
        NEES_add.FeaturePosition(i)=NEES_add.FeaturePosition(i)+error(3*j+4:3*j+5)'/X_Estimation{i}.cov(2*j+5:2*j+6,2*j+5:2*j+6)...
