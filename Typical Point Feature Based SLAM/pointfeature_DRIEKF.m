function X_estimation=pointfeature_DRIEKF(X0,P0,z0,U_noise,z_noise,Index,ODOM_noise,OBSV_noise)

% coded by Yang SONG

tic
T_steps=size(Index,2);
[Xn,Pn]=Augmentation_DRI_point(X0,P0,z0,OBSV_noise);
...
for i =1:T_steps
    ...
    A(4:6,1:3)=skew( X_Prediction(1:3,4) )*Xn(1:3,1:3);%this is for the first-order integration noise
    ...
    %The form of error xi is a 6(N+1)*1 column (xi_R,xi_Rp,xi_x,xi_p)
    for j=1:N_ob
        H(3*j-2:3*j,1:3)=X_Prediction(1:3,1:3)'*skew(Xstate4Jacobian(1:3,4+Index{i}.RemainIndex(j)));
...
%         Omega(3*N_ob+3*j-2:3*N_ob+3*j,3*N_ob+3*j-2:3*N_ob+3*j)=OBSV_noise(4:6,4:6);
    ...
    Xn=poseState_DRI_plus_point(posestate_DRI_exp_point(K*Y),X_Prediction);
    ...
    if size(z_noise{i}.position(:,N_ob+1:end),2)>0.5
        ...
        [Xn,Pn]=Augmentation_DRI_point(Xn,Pn,z_new,OBSV_noise);
        Pn=(Pn+Pn')/2;
