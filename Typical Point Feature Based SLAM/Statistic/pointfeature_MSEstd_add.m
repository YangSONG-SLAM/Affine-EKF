...
function MSE_add=pointfeature_MSEstd_add(X_Estimation,Xstate_gt,m)
...
for i=1:T_steps
    error=(pointState_Std_minus(X_Estimation{i}.state,Xstate_gt{i})).^2/m;
...
%     MSE_add.FeatureRotation(i)=sum(error(4:3*N+3));
    MSE_add.FeaturePosition(i)=sum(error(7:3*N+6))/(N);
