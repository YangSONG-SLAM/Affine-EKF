%coded by Yang SONG
function [z_noise,z0]=zaddNoise_point(z_expectation,z_expectation0,OBSV_noise)
...
%         z0.rotation(1:3,3*j-2:3*j)=so3_exp(V(1:3))*z_expectation0.rotation(1:3,3*j-2:3*j);
        z0.position(1:3,j)=z_expectation0.position(1:3,j)+V;
    end
end
...
%             z_noise{i}.rotation(1:3,3*j-2:3*j)=so3_exp(V(1:3))*z_expectation{i}.rotation(1:3,3*j-2:3*j);
            z_noise{i}.position(1:3,j)=z_expectation{i}.position(1:3,j)+V;
        end
    end
end
