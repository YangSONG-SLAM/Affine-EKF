function X_estimation=CPplanefeature_AffEKF(X0,P0,z0,U_noise,z_noise,Index,ODOM_noise,OBSV_noise)
...
%     for j=1:N
%         n_f_pre=X_Prediction(1:3,4+j)/norm(X_Prediction(1:3,4+j));
%         n_f_up=Xn(1:3,4+j)/norm(Xn(1:3,4+j));
%         AA(4+3*j:6+3*j,1:3)=skew( X_Prediction(1:3,4+j)-Xn(1:3,4+j) )-(n_f_pre*n_f_pre'-n_f_up*n_f_up')*skew(X_Prediction(1:3,4));
%         AA(4+3*j:6+3*j,4:6)=-(n_f_pre*n_f_pre'-n_f_up*n_f_up');
%     end
    for j=1:N
        n_f_pre=X_Prediction(1:3,4+j)/norm(X_Prediction(1:3,4+j));
        n_f_up=Xn(1:3,4+j)/norm(Xn(1:3,4+j));
