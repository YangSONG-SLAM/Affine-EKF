function X_estimation=CPplanefeature_idealEKF(X0,P0,z0,U_noise,z_noise,Index,ODOM_noise,OBSV_noise,Xstate_gt)
...
i=1;
    ...
    %Fn
%     R=Xstate_gt{i}(1:3,1:3);
%     pr=Xstate_gt{i}(1:3,4);
    F=eye(3*N+6);
...
%         H(3*j-2:3*j,3*Index{i}.RemainIndex(j)+1:3*Index{i}.RemainIndex(j)+3)=X_Prediction(1:3,1:3)';
        ...
        d_f=norm(Xstate_gt{i}(1:3,4+Index{i}.RemainIndex(j)));
        n_f=Xstate_gt{i}(1:3,4+Index{i}.RemainIndex(j))/d_f;
...
%         [Xn,Pn]=Augmentation_Std_CPplane(Xn,Pn,z_new,OBSV_noise);
        [Xn,Pn]=Augmentation_Ideal_CPplane(Xn,Pn,z_new,OBSV_noise,Xstate_gt{i});
