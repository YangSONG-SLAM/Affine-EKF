%increment of NEES for plane feature based SLAM
...
function NEES_add=planefeature_NEES_add(X_Estimation,Xstate_gt,m)
...
NEES_add.Feature_NormV=zeros(1,T_steps);
NEES_add.Feature_dV=zeros(1,T_steps);
NEES_add.Feature=zeros(1,T_steps);

for i=1:T_steps
    error=planeState_Std_minus(X_Estimation{i}.state,Xstate_gt{i});
    ...
    for j=1:N
        NEES_add.Feature_NormV(i)=NEES_add.Feature_NormV(i)+error(3*j+4:3*j+5)'/X_Estimation{i}.cov(3*j+4:3*j+5,3*j+4:3*j+5)...
            *error(3*j+4:3*j+5)/(m*2*N);
        NEES_add.Feature_dV(i)=NEES_add.Feature_dV(i)+error(3*j+6)'/X_Estimation{i}.cov(3*j+6,3*j+6)...
            *error(3*j+6)/(m*N);
        NEES_add.Feature(i)=NEES_add.Feature(i)+error(3*j+4:3*j+6)'/X_Estimation{i}.cov(3*j+4:3*j+6,3*j+4:3*j+6)...
